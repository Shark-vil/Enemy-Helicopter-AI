...
function ENT:FindControlPos()
	...
	while ( true ) do
		if ( #players ~= 0 and math.random( 0, 1 ) == 1 ) then
			...
			vec = Vector( helPos.x + math.random(-10000, 10000), helPos.y + math.random(-10000, 10000), helPos.z + math.random(-100, 100) );
		end;
...
function ENT:MissileEscape(phy)
	local objects = ents.FindInSphere( self:GetPos(), 1000 );
	local ang = nil;
	for i = 1, #objects do
		if ( objects[i] ~= self and objects[i]:GetClass() == "rpg_missile" ) then
...
function ENT:Find()
	...
	if ( j ~= 0 ) then
		for i = 1, j do
			...
			if ( target:GetClass() ~= self.NoTargetClass ) then
...
function ENT:CustomNavigationMove(target, phy)
	...
	local tr = util.TraceLine( {
		...
		filter = function( ent ) 
			if ( ent:GetClass() ~= "npc_apache_scp_sb" ) then 
				return true;
			end;
		end;
	} );
...
hook.Add("EntityTakeDamage", "npc_apache_scp_sb_EntityTakeDamage", function( ply, dmg )
	...
	if ( class == "npc_apache_scp_sb" or class == "proj_dan_heli_shot_scp_sb_fg" ) then
